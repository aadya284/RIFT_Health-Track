...
function generateMockReport(drugs, filename, submittedAt) {
  ...
  const drugMap = {
    ...
    Fluorouracil: { riskLabel: 'Toxic', severity: 'High', confidence: '96.0%' },
  }
...
export default function Results() {
  ...
  const { filename, drugs = ['Codeine', 'Warfarin', 'Clopidogrel'], submittedAt } = state

  const report = generateMockReport(drugs, filename, submittedAt)
      ...
      <section aria-labelledby="interpretation-heading" className="mb-6">
        ...
        <div className="space-y-1">
          <AccordionSection title="Summary" defaultOpen={true}>
            <p className="text-sm text-gov-text-secondary leading-relaxed m-0">
              {report.llmInterpretation.summary}
            </p>
          </AccordionSection>

          <AccordionSection title="Mechanism of Action">
            <p className="text-sm text-gov-text-secondary leading-relaxed m-0">
              {report.llmInterpretation.mechanism}
            </p>
          </AccordionSection>

          <AccordionSection title="Variant Citations">
            <ul className="list-none p-0 m-0 space-y-2">
              {report.llmInterpretation.variantCitations.map((cite, idx) => (
          ...
          <AccordionSection title="Clinical Significance">
            <p className="text-sm text-gov-text-secondary leading-relaxed m-0">
              {report.llmInterpretation.clinicalSignificance}
            </p>
          </AccordionSection>
        </div>
      </section>
