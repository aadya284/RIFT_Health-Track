...
# Drug to gene mappings using uppercase keys (for case-insensitive matching)
...
# Cached frozenset of known drugs for O(1) lookup
...
class RiskEngine:
    ...
    def get_risk_and_severity(drug: str, phenotype: str) -> Tuple[str, str]:
        ...
        Returns:
            ...
            severity: none, low, moderate, high, critical
        """
        # Handle unknown phenotype
        if phenotype == "Unknown":
            logger.debug(f"Risk lookup: {drug} + Unknown phenotype, returning Unknown")
            return ("Unknown", "none")
        
        # Direct tuple lookup in RISK_MAPPINGS (O(1))
        ...
        # Default fallback (should not reach here if mapping is complete)
        logger.debug(f"Risk lookup: {drug} + {phenotype} not found, defaulting to Safe/none")
        return ("Safe", "none")
    ...
    def get_gene_for_drug(drug: str) -> str:
        ...
        Returns:
            Gene name
        """
        return DRUG_GENE_MAPPING.get(drug, "UNKNOWN")
    
    @staticmethod
    def is_known_drug(drug: str) -> bool:
        """Check if drug is in our knowledge base (O(1) frozenset lookup)."""
        return drug in _KNOWN_DRUGS
